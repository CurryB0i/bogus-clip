...
const EditCaptions = () => {
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (e: KeyboardEvent) => {
      if (
        !focusedEventRef.current?.contains(e.target as Node) &&
  ...
  useEffect(() => {
    ...
    const event =
      state.transcript.events.find((e) => time >= e.start && time <= e.end) ??
      null;
    ...
    (event: Event) => {
      if (videoRef.current && canonical) {
        videoRef.current.currentTime =
          canonical.wordsById[event.wordIds[0]].start;
      }
    },
    [canonical],
  );
  ...
  const handleAddEvent = useCallback(
    (event: Event) => {
      ...
      setCanonical((prev) => {
        ...
        const newWordId = Math.max(...Object.keys(wordsById).map(Number)) + 1;

        const newEventId = Math.max(...Object.keys(events).map(Number)) + 1;
    ...
    (event: Event) => {
      ...
      event.wordIds.forEach((id) => {
        ...
        if (cleaned.length === 0) {
          ...
          if (cleaned.length > 1) {
            const nextWord =
              newEvents[eventId].wordIds[
                newEvents[eventId].wordIds.indexOf(id) + 1
              ];
      ...
      const prevWordId =
        prevEventId === undefined ? -1 : newEvents[prevEventId].wordIds.at(-1)!;
      const nextEventId = canonical.eventOrder[eventIndex + 1];
      const nextWordId =
        nextEventId === undefined ? -1 : newEvents[nextEventId].wordIds[0];
      const start = prevWordId === -1 ? 0 : newWordsById[prevWordId].end;
      const end =
        nextWordId === -1
          ? videoRef.current!.duration
          : newWordsById[nextWordId].start;
      ...
      if (draftStart < start) {
        ...
        newWordsById[updatedEvent.wordIds[0]].start =
          newWordsById[updatedEvent.wordIds[0]!].start;
      } else {
        newWordsById[updatedEvent.wordIds[0]].start = draftStart;
      }
      if (draftEnd < draftStart) {
        newWordsById[updatedEvent.wordIds.at(-1)!].end =
          newWordsById[updatedEvent.wordIds.at(-1)!].end;
  ...
  const handleReset = async () => {
    ...
    const withoutExt = state.file.name.substring(
      0,
      state.file.name.lastIndexOf("."),
    );
    const orgTranscript = await getTranscriptFromDB(
      "T_" + withoutExt + "_original",
    );
    if (orgTranscript) {
      const newTranscript: Transcript = {
        ...orgTranscript,
  ...
  return (
    <div className="w-full h-[90%] flex flex-col items-center justify-center">
      <div className="w-full h-full flex items-center justify-center">
        <div className="w-[60%] h-full flex border-4 border-blue-500 py-5 rounded-lg">
          <div
        ...
        <div className="relative w-1/3 flex items-center justify-center">
          ...
          <div
            className="absolute -bottom-15 font-bold bg-white p-2 border-2
